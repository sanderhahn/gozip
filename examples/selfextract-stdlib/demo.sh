#!/bin/bash
# Demonstrates a self-extracting binary using only the Go standard library.
#
# This script:
#   1. Creates sample payload files
#   2. Builds the selfextract binary (stdlib only, no gozip import)
#   3. Appends payload files to the binary using the gozip CLI
...
echo '{"app": "selfextract-stdlib-demo", "version": "1.0"}' > payload/config.json
mkdir -p payload/templates
echo "<h1>Welcome</h1>" > payload/templates/index.html
echo "Payload files created."

echo ""
echo "=== Step 2: Build the selfextract binary (stdlib only) ==="
